...
async function handleToolsList(request, env) {
  ...
  const listStatement = env.DB.prepare(listSQL).bind(...binds, pageSize, offset);
  const countStatement = env.DB.prepare(countSQL).bind(...binds);

  const [listResult, countResult] = await Promise.all([listStatement.all(), countStatement.first()]);
...
export default {
  async fetch(request, env) {
    ...
    return withSecurityHeaders(assetResponse, "public, max-age=300");
  },
};
